...
// TestOverrunSequenceItemSurfacesWarning is the file-level form of keeping a
// sequence item whose declared length overruns the sequence: the item is in
// the data set and DICOMFile.Warnings names the overrun.
func TestOverrunSequenceItemSurfacesWarning(t *testing.T) {
	itemBody := explicitElement(0x0008, 0x0100, "SH", []byte("CODE02  "))
	var item1, item2 bytes.Buffer
	item1.Write(explicitElement(0x0008, 0x0100, "SH", []byte("CODE01  ")))
	item2.Write(itemBody)

	rawItem := func(length uint32, body []byte) []byte {
		var buf bytes.Buffer
	...
	overrun := uint32(24)
	seqItems := append(rawItem(uint32(item1.Len()), item1.Bytes()),
		rawItem(uint32(item2.Len())+overrun, item2.Bytes())...)
	...
	_ = binary.Write(&seq, binary.LittleEndian, uint32(len(seqItems)))
	seq.Write(seqItems)
	...
	df := readFile(t, buildFile("1.2.840.10008.1.2.1", ds.Bytes(), false))
	if len(df.DataElements) != 2 {
		t.Fatalf("kept %d top-level elements, want 2", len(df.DataElements))
	}
	seqElem := df.DataElements[1]
	if len(seqElem.Items) != 2 {
		t.Fatalf("sequence has %d items, want 2 — overrun last item was dropped", len(seqElem.Items))
	}
	if got := string(seqElem.Items[1].Elements[0].Value); got != "CODE02  " {
		t.Errorf("item 1 = %q, want CODE02  ", got)
	}
	if len(df.Warnings) == 0 {
		t.Fatal("expected DICOMFile.Warnings to record the overrun item length")
	}
}
